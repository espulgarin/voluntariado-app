...
Class Action {
	...
	function guardar_categoria()
	{
		...
		foreach($_POST as $k => $v)
		{
			if(!in_array($k, array('id_categoria')) && !is_numeric($k))
			{
				if(empty($data))
				{
					$data .= " $k='$v' ";
		...
		{
			$save = $this->db->query("INSERT INTO categorias set $data");
		}
		else
		{
			$save = $this->db->query("UPDATE categorias set $data where id_categoria = $id_categoria");
		}
		if($save)
		{
			return 1;
	...
	function eliminar_categoria(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM categorias where id_categoria = $id");
		if($delete){
			return 1;
		}
	}
