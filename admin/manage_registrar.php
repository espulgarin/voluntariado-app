...
<div class="container-fluid">
	<form action="" id="ar-frm">
		...
		<div class="form-group">
			<label for="">Registradores</label>
			<select name="user_id[]" id="user_id" multiple="multiple" class="custom-select custom-select-sm select2" >
				<option value=""></option>
				<?php 
				$uqry = $conn->query("SELECT *,concat(firstname,' ',lastname) as name FROM users where type = 2 order by concat(lastname,', ',firstname) asc");
...
<script>
	...
	$('.select2').select2({
		placeholder:"Por favor, seleccione un registrador",
		width:"100%"
	})
