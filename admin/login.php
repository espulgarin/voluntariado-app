...
<head>
  ...
  <title>Inicio de sesión | Voluntariado</title>
...
<body class="bg-dark">
  ...
  <main id="main" >
		...
		<h4 class="text-white text-center"><b>Sistema de registro de voluntariado</b></h4>
	...
	$('#login-form').submit(function(e){
		...
		$('#login-form button[type="button"]').removeAttr('disabled').html('Login');
			...
			success:function(resp)
			{
				if(resp == 1)
				{
					location.href ='index.php?page=inicio';
				}
				else
				{
					 alert(resp);
				return resp;  
					$('#login-form').prepend('<div class="alert alert-danger">El usuario o la contraseña ingresados son incorrectos.</div>')
					$('#login-form button[type="button"]').removeAttr('disabled').html('Login');
				}
			}
		})
